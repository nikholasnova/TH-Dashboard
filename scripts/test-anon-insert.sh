...
CANARY_DEVICE="test_canary_migration_01"
PASS=0
FAIL=0
...
check "normal sensor reading"          "{\"device_id\":\"$CANARY_DEVICE\",\"temperature\":22.47,\"humidity\":48.91}"  ok
check "cold but valid"                  "{\"device_id\":\"$CANARY_DEVICE\",\"temperature\":-20.0,\"humidity\":30}"    ok
check "hot but valid"                   "{\"device_id\":\"$CANARY_DEVICE\",\"temperature\":55.0,\"humidity\":80}"     ok
...
check "temperature below -50"           "{\"device_id\":\"$CANARY_DEVICE\",\"temperature\":-999,\"humidity\":50}"           blocked
check "temperature above 100"           "{\"device_id\":\"$CANARY_DEVICE\",\"temperature\":500,\"humidity\":50}"            blocked
check "humidity above 100"              "{\"device_id\":\"$CANARY_DEVICE\",\"temperature\":22,\"humidity\":150}"            blocked
check "humidity negative"               "{\"device_id\":\"$CANARY_DEVICE\",\"temperature\":22,\"humidity\":-5}"             blocked
check "attacker sets deployment_id"     "{\"device_id\":\"$CANARY_DEVICE\",\"temperature\":22,\"humidity\":50,\"deployment_id\":1}" blocked
check "attacker sets source=weather"    "{\"device_id\":\"$CANARY_DEVICE\",\"temperature\":22,\"humidity\":50,\"source\":\"weather\"}" blocked
check "attacker sets observed_at"       "{\"device_id\":\"$CANARY_DEVICE\",\"temperature\":22,\"humidity\":50,\"observed_at\":\"2020-01-01T00:00:00Z\"}" blocked
check "attacker sets zip_code"          "{\"device_id\":\"$CANARY_DEVICE\",\"temperature\":22,\"humidity\":50,\"zip_code\":\"85142\"}" blocked
...
echo
echo "All good. Canary rows were inserted with device_id='$CANARY_DEVICE'."
echo "Clean them up when you're done:"
echo "  DELETE FROM readings WHERE device_id = '$CANARY_DEVICE';"
