/*
 * IoT Temperature & Humidity Sensor Node — ESP32-S3 variant
 *
 * Hardware: ESP32-S3-WROOM-1, DHT20 (I2C), 16x2 LCD (parallel)
 *
 ...
 *
 * Wiring (ESP32-S3 GPIOs):
 *   DHT20:  VCC->3.3V, SDA->GPIO8, GND->GND, SCL->GPIO9
 *   LCD:    RS->GPIO4, EN->GPIO5, D4->GPIO6, D5->GPIO7, D6->GPIO15, D7->GPIO16
 *           VSS->GND, VDD->5V, V0->pot center, RW->GND,
 *           LED+->1k resistor->5V, LED- ->GND
 */
...
void setup() {
  ...
  Serial.println();
  Serial.println("=== IoT Temp/Humidity Sensor (ESP32-S3) ===");
...
void loop() {
  ...
  if (now - lastSendTime >= SEND_INTERVAL_MS && readingCount > 0) {
    ...
    if (sent) {
      ...
      consecutiveFailures++;
      unsigned long backoff = min((unsigned long)30000 * consecutiveFailures, (unsigned long)SEND_INTERVAL_MS);
