...
Supabase also gave me Postgres features that are neat: RPC functions for aggregation queries (`get_device_stats`, `get_chart_samples`, `get_dashboard_live`, `get_report_bundle`), Row Level Security for fine-grained access control, exclusion constraints for preventing overlapping deployment windows, and partial unique indexes for weather deduplication. The schema defines the full set of tables (readings, deployments, devices, app_settings, device_alert_state, user_roles, cron_runs, role_change_audit), 9 RPC functions, 3 constraint checks, and multiple index strategies. All of this runs on the database side, which means the web app stays thin.
...
### Why a Chat-Driven `.tex` Report Generator (replacing in-browser Pyodide)

The original analysis page ran Python (numpy, pandas, scipy, statsmodels) in the browser via Pyodide. It worked, but in practice students almost never used it — the cold start was 10-15 seconds for a CDN-side Pyodide download, the statistical outputs (ANOVA, Tukey HSD, Holt-Winters forecasts) were well beyond what the class paper needed, and whatever they produced still had to be copy-pasted somewhere else to become a submittable document. It felt like the feature existed for its own sake rather than for the actual user.

What students actually wanted was a report they could submit. So I replaced the analysis page with a chat-driven LaTeX report generator. Flow: the user asks the floating AI chat for a report, the AI asks for a date range, then calls a `prepare_report` tool. The server runs a single `get_report_bundle` RPC (more on that below), caches the bundle in Redis, and writes a `__QUESTION__<json>` marker into the chat response stream. The client parses the marker, opens a modal with pre-filled defaults (title, author from Supabase auth, institution, devices to include, etc.), and on submit posts directly to `/api/reports/generate` — deliberately bypassing the chat Gemini so the user isn't gated on another turn of tool-calling. Generation runs `get_report_bundle` again if the user narrowed device selection, calls Gemini 2.5 Flash once in isolation with structured output to produce the analytical prose, and assembles the full `.tex` via a pure-TS template (`reportTemplate.ts`). Delivery is two buttons in an inline `ReportArtifactCard`: **Download .tex** (client-side blob) and **Open in Overleaf** (inline form POST with the raw tex in the `snip` field, which opens a new Overleaf project with the source pre-loaded — Overleaf's free tier compiles the PDF on their infrastructure in seconds).

A few decisions worth calling out. Gemini gets used twice in two very different roles: the main chat Flash is just an intent router plus tool caller, and a separate isolated Flash call inside `/api/reports/generate` produces the analytical prose as a JSON object with one field per narrative section (coverage, statistical summary, diurnal, accuracy, error trend, outliers, key findings). Every narrative slot has a deterministic fallback built from the same numbers, so if Flash fails, times out, or violates a forbidden-phrase filter (any causal, forward-looking, hardware, or placement speculation gets the field nulled), the template substitutes deterministic bullet summaries and the report still ships. The `latexEscape` function is a single-pass regex over LaTeX specials followed by Unicode substitutions (Greek letters, `°`, em-dashes, etc.); the order matters because the substitutions produce LaTeX macros with braces that shouldn't be re-escaped. All of the heavy pgfplots work — the multi-node diurnal overlay, per-device daily min/max shaded bands, distribution histograms — is deterministic tikz/pgfplots output from the bundle, not AI-generated.

The `get_report_bundle` RPC is the core data engine here. One call returns everything the template needs: scoped deployments, per-deployment and overall stats (with median and stddev), hourly-of-day averages in Phoenix time for the diurnal chart, per-device hourly + daily breakdowns (for multi-node overlays and the daily-range shaded bands), daily sensor-vs-weather comparison with percent error, Pearson r between temperature and humidity, IQR outliers on daily averages, a `devices_info` payload for the per-device colour palette, and gap detection (>3h between consecutive readings within an active deployment window). Getting everything server-side in one round trip keeps the generation flow snappy and means the template code is a pure function of the bundle.

The biggest trade-off compared to the old Pyodide page is that I no longer render statistical plots in the browser — everything happens at PDF generation time via pgfplots. In exchange: zero cold-start cost, no ~15MB WASM download, and the output is a real report rather than screenshots of in-browser charts.

### Why Gemini for AI Chat

Google Gemini 2.5 Flash was my choice for the AI chat because it offers a free API tier with good function-calling support and streaming. The chat uses 9 tools (`get_deployments`, `get_deployment_stats`, `get_readings`, `get_device_stats`, `get_chart_data`, `get_report_data`, `get_report_bundle`, `prepare_report`, `get_weather`) that give the AI access to all project data through the same Supabase queries the dashboard uses. The tools execute via `aiTools.ts` with a service-role Supabase client, so the AI can query data without being limited by the browser session. `prepare_report` is the entry point for the chat-driven report flow described earlier — it's special-cased in the chat route to emit a `__QUESTION__<json>` marker directly into the stream.
...
The `DevicesContext` provides the active device list app-wide via React context. It fetches devices on mount (filtered by `is_active = true`, ordered by `sort_order`) and exposes a refresh function that the `DeviceManager` calls after adding or editing a device. The dashboard, charts, compare, and data explorer pages all read from this context rather than making independent device queries. This ensures that when a device is deactivated in the manager, it immediately disappears from all pages without requiring a full page reload.
...
Without deployments, all readings from `node1` would be lumped together regardless of where the sensor was physically located. Deployments scope the data so that analysis is meaningful: when the report bundle computes per-deployment stats or sensor-vs-weather error, it's comparing distinct physical placements rather than random time slices from different environments. The chat-driven report generator also uses `devices_info` from the bundle to give each deployment its own colour in multi-node charts.

The database enforces several integrity constraints: device IDs must be registered sensor devices, deployment text fields and ZIP codes have bounded formats, ended deployments must end after they start, a unique partial index (`idx_deployments_one_active_per_device`) prevents multiple active deployments per device where `ended_at IS NULL`, and an exclusion constraint using `btree_gist` (`deployments_no_overlap_per_device`) prevents overlapping time windows per device using `tstzrange` overlap checks. A trigger also blocks changing a deployment's `device_id` after creation, which matters because authenticated users can edit their own deployment metadata directly through RLS.
...
The report generator is the feature that replaced the Pyodide analysis page. The user asks the floating AI chat for a report (e.g. "generate a report" or "make a data report for my paper"), the chat asks for a date range if one wasn't provided, and once it has one, it calls `prepare_report`. The tool hits `get_report_bundle` against Supabase, caches the full bundle in Redis with a 30-minute TTL, and emits a `__QUESTION__<json>` marker into the chat response stream carrying the context id, pre-fill defaults, and an `available_devices` list. The client parses the marker to open `ReportOptionsModal`; the modal lets the user adjust title/author/institution, toggle which devices to include (checkbox list, defaults to all), toggle the gap-annotation and split-by-device and weather-section options, and submit. Submission POSTs `/api/reports/generate` directly — the chat Gemini is NOT in this second round trip.

`/api/reports/generate` does four things. First, it validates the user and checks `reportLimiter` (5 per hour per user). Second, if the user narrowed device selection relative to the cached bundle, it re-fetches a device-scoped bundle so stats reflect only the selected nodes. Third, it runs `generateReportProse` — a single isolated Gemini 2.5 Flash call with structured JSON output, a tight system prompt, a forbidden-phrase filter, and an 8-second timeout — which returns analytical narrative for each section. Fourth, it builds the full `.tex` via `buildTexSource` (pure function, no I/O), stores the tex in Redis keyed by a 128-bit UUID with 30-min TTL, and returns `{ report_id, filename, byte_size }`.

The template itself is deterministic TeX/pgfplots. Sections: cover page (uses the `titling` package so `\droptitle=-4em` pulls the block up and tightens the first-page dead space), Data Collection with a deployment table, Statistical Summary (callout box of mean/median/range/stddev + Pearson r, then prose or deterministic fallback bullets), Diurnal Patterns (single-series chart for one device, or a multi-node overlay with solid temperature lines and dashed humidity lines on a dual Y axis when there are 2+ devices), Daily Range (per-device mean line with a translucent shaded band between daily min and max, via the `fillbetween` pgfplots library), Distribution (overlaid histograms of hourly-average temperature and humidity, 5-unit bins, per-device colours), Sensor Accuracy (conditional on weather presence — error callout + daily sensor-vs-weather charts + error-trend chart with a zero reference line), Key Findings bullets (AI or deterministic), and Appendices A/B/C with full hourly and daily comparison tables capped at 60 evenly-sampled rows so large windows don't blow Overleaf's free-tier 20-second compile budget.

Getting the LaTeX right involved a lot of small fixes that are easy to get wrong:
- **Single-pass escape.** `latexEscape` is a single regex over `\ & % $ # _ { } ~ ^` first, then Unicode substitutions (Greek letters, degree signs, em dashes, math symbols) second. Order matters because the Unicode substitutions emit `\textdegree{}`-style macros whose braces must NOT be re-escaped; earlier two-pass version doubled-escaped `{}` into `\{\}` and produced literal `\textdegree{}F` strings in the compiled PDF.
- **T1 fontenc + emergency stretch.** Without `\usepackage[T1]{fontenc}`, `<` and `>` in text mode render as `¡` and `¿` (an OT1-encoding ligature issue that bit the "gaps >3h" section header). `\emergencystretch=3em` plus `\sloppy` absorb the ~5pt overfull hbox warnings that pgfplots legends otherwise produce.
- **Widow/orphan guards.** `\widowpenalty=10000`, `\clubpenalty=10000`, `\raggedbottom`, and `\needspace{N\baselineskip}` before every `\section*` prevent section titles from orphaning at the bottom of pages and floaty-looking page breaks mid-figure. `\clearpage` before Appendix A because the raw-data tables are a clear document division.
- **Phoenix timezone in SQL.** The `get_report_bundle` RPC casts to Phoenix local time before extracting hour-of-day and date buckets, so the diurnal chart shows 56°F at 6 AM and 84°F at 3 PM rather than being UTC-shifted.
- **Dates in the template.** `parseCalendarDate` parses `YYYY-MM-DD` strings from SQL as local calendar dates rather than UTC midnight, so a Phoenix date of 2026-04-12 in the SQL doesn't render as "Apr 11" after timezone conversion.

The AI prose itself is heavily constrained. The `reportProse.ts` system prompt forbids causal speculation ("because", "due to", "caused by", etc.), forward-looking or "limitations" language ("future work", "would benefit", "recommend"), hardware/placement references ("wall", "enclosure", "DHT20", "Arduino"), editorializing adjectives, and Greek letters. A regex `FORBIDDEN_PATTERNS` list post-filters each field and nulls anything that slips through. Every narrative field in the `ReportProse` schema is nullable, and the template has a deterministic bullet-list fallback for each section computed directly from the bundle numbers — so when Flash times out, hallucinates, or trips the filter, the report still renders with accurate content.

The generated tex is handed to Overleaf via an inline form POST rather than via `snip_uri`. Early versions used `snip_uri` pointing at `/api/reports/[id]/tex`, but that only works if Overleaf's servers can reach the URL — which they can't during local dev and can't inside any authenticated Vercel preview. The fix was to fetch the tex client-side, construct a hidden form with `snip` (raw tex content), `engine=pdflatex`, and `snip_name` (the filename), and submit it to `https://www.overleaf.com/docs` with `target=_blank`. Overleaf creates a new project on their side containing the tex, free account required. The Download button does the same client-side fetch and pipes it through a blob URL so the `.tex` ends up in the user's downloads folder.

### AI Chat with Tool Calling

The floating `ChatShell` component is mounted in the root layout (`layout.tsx`), making it available on every authenticated page without remounting during navigation. It sends messages to `POST /api/chat`, which authenticates the user via `getServerUser()`, checks an Upstash Redis rate limit (sliding window — 30 requests per 15 min per signed-in user), and forwards the conversation to Gemini 2.5 Flash. The limiter fails closed in production if Upstash env vars are missing.
...
The `maxDuration` is set to 120 seconds (the Vercel serverless function timeout for the Pro plan) to accommodate multi-step tool loops. For the chat-driven `.tex` report flow, the chat Gemini only runs intent detection plus one `prepare_report` call; the actual report build happens in `/api/reports/generate`, which has its own 60-second ceiling and runs one isolated Gemini prose call plus a pure-TS template assembly. Keeping the two Gemini calls separate — one for conversational tool routing, one for structured analytical prose with strict rules — turned out to be materially more reliable than trying to get the chat model to hold the whole report in one turn.
...
### Constraining the Report Prose LLM

The hardest part of the `.tex` report wasn't the LaTeX — it was keeping the AI prose truthful and submittable without a human in the loop. Gemini Flash will happily write "the sensor was likely near a wall, which explains the warm readings" from one number. That's a causal claim the model has no grounds for, and it's the kind of thing that makes a class paper look silly.

Three defenses stack on top of each other. First, the system prompt for `generateReportProse` includes explicit forbidden-language rules: no "because", "due to", "likely", "probably", "suggests", no hardware references (DHT20, Arduino, WiFi, wall, patio, enclosure), no forward-looking content ("future work", "limitations", "recommend"), no editorializing adjectives ("notable", "impressive"). The prompt is clear that the output is consumed verbatim — it has to be a finished draft, not a scaffolding with gaps for the student to fill in.

Second, the Gemini call uses structured output (`responseMimeType: 'application/json'` + `responseSchema`) so the model can only return a specific JSON shape with one field per section. This stops the model from returning narrative outside the expected structure.

Third, after the response comes back, each field runs through a post-filter that checks against a list of forbidden-phrase regexes. Any field that matches anything — a single `\blikely\b` or `\bshould explore\b` is enough — gets nulled. Bullets are filtered individually, and any bullet mentioning the forbidden list is dropped from the array. This is the last line of defense and catches cases where the model ignores the system prompt.

Every nullable field in `ReportProse` has a deterministic bullet-list fallback that the template substitutes. The fallbacks are built from the same bundle numbers the prose was meant to describe ("Temperature averaged 77.3°F with a standard deviation of 8.46°F", etc.), so the report always has accurate content — it just reads as a short bullet list instead of a full paragraph when the AI fails or is rejected. This turned out to be the right trade-off: a slightly terser report that's always truthful, rather than a prettier report that occasionally hallucinates.
...
Row Level Security policies needed to serve three different access patterns simultaneously: Arduino devices (anon role) need INSERT on `readings`, authenticated dashboard users need SELECT on `readings`, users need owner-scoped deployment edits, admins need device management and destructive cleanup, and server-side routes need unrestricted access for weather inserts, keepalive state upserts, and cascade deletes.

The solution was careful per-table RLS policies. The `readings` table has three policies: a validated anon INSERT (`WITH CHECK` enforcing a registered active device ID, temp/humidity bounds, `source = 'sensor'`, and null-enforcement on `deployment_id`/`zip_code`/`observed_at` so attackers can't backfill or mislabel rows), an authenticated SELECT gated on `auth.uid() IS NOT NULL`, and a service_role DELETE policy. Deployments have signed-in SELECT, owner-scoped INSERT/UPDATE, and admin-only DELETE. Devices remain admin-only for writes. Admin checks look up `user_roles` directly on `auth.uid()` rather than trusting the JWT claim, so role changes take effect immediately. The `device_alert_state` table has authenticated SELECT only, writes go through service_role (which bypasses RLS). Originally the readings anon INSERT was `WITH CHECK (true)` and deployments had a single authenticated "allow all" policy; both were tightened in later hardening passes.
...
**Self-hosted LaTeX compile for the reports.** The report generator currently hands the `.tex` off to Overleaf via an inline form POST, which works great but requires a free Overleaf account to actually produce the PDF. A small Docker sidecar (e.g. `texlive/texlive:latest-basic` on Fly.io or Render) running a ~30-line HTTP wrapper around `pdflatex` would let us return a PDF directly without sending users to a third party. It's additive to the current design — nothing about the template or the chat flow would change. I deliberately did not build it because Vercel can't bundle TeX Live (5GB+ vs the 250MB serverless function limit), and a separate deploy target felt like unwarranted ops complexity for a class project. If Overleaf signup friction turns out to matter, it's maybe an afternoon's work.
