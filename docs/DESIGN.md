...
The solution was careful per-table RLS policies. The `readings` table has three policies: an anon INSERT with `WITH CHECK (true)`, an authenticated SELECT gated on `auth.uid() IS NOT NULL`, and a service_role DELETE policy. Deployments and devices use a single "allow all" policy for authenticated users. The `device_alert_state` table has authenticated SELECT only, writes go through service_role (which bypasses RLS).
